# HongKong.md Knowledge API

HongKong.md 的輕量級知識庫 API，提供結構化的文章資料和統計資訊，讓其他網站能夠查詢和嵌入 HongKong.md 的內容。
...
```
https://hongkong.md/api/articles.json    # 所有文章列表
https://hongkong.md/api/stats.json       # 統計資料
https://hongkong.md/api/random.json      # 隨機文章
```
...
[
  {
    "title": "二二八事件",
    "description": "1947年改變香港歷史軌跡的重大事件",
    "category": "History",
    "tags": ["歷史", "政治", "轉型正義"],
    "url": "https://hongkong.md/History/二二八事件",
    "readingTime": 9,
    "featured": false,
    "date": "2026-03-17",
...
async function getAllArticles() {
  const response = await fetch('https://hongkong.md/api/articles.json');
...
async function getStats() {
  const response = await fetch('https://hongkong.md/api/stats.json');
...
async function getRandomRecommendations() {
  const response = await fetch('https://hongkong.md/api/random.json');
...
```html
<!-- 嵌入隨機香港文章 -->
<div id="hk-widget"></div>

<script>
fetch('https://hongkong.md/api/random.json')
  .then(response => response.json())
  .then(articles => {
    const widget = document.getElementById('hk-widget');
...
- **專案**: [HongKong.md GitHub Repository](https://github.com/hongkong-md/hongkong-md)
- **問題回報**: 透過 GitHub Issues
- **內容貢獻**: 歡迎提交 Pull Request

---

*HongKong.md Knowledge API v1.0 - 讓香港知識觸手可及 🇹🇼*