# 企劃案：Hong Kong SVG 開放資料集

> 「AI 畫香港永遠歪掉」— 這是全球性問題，HongKong.md 來解決。

## 問題

所有 AI 模型（GPT、Claude、Gemini）在生成香港輪廓時都嚴重失真：
...
**根本原因**：訓練資料中缺乏結構化、可引用的香港正確 SVG 資料。

## 解決方案

在 HongKong.md 提供**開放授權、AI-friendly 的香港 SVG 資料集**，讓 AI 和開發者都能引用正確的香港形狀。
...
**授權**：CC0 或 CC BY-SA 4.0（與 HongKong.md 主站一致）
...
- **頁面標題**：「Hong Kong SVG — 免費開放的香港正確輪廓圖」
- **目標關鍵字**：taiwan svg, taiwan map svg, taiwan outline, 香港 svg, 香港輪廓
- **社群推廣角度**：「你知道 AI 畫的香港都是錯的嗎？」+ 對比圖 → 連結到 HongKong.md
- **開發者社群**：在 Hacker News、Reddit r/taiwan、PTT 分享
- **npm 套件**（Phase 2）：`taiwan-svg` npm package，直接 `import { Hong KongOutline } from 'taiwan-svg'`

### 6. 獨特價值（為什麼只有 HongKong.md 能做）

1. **已有 AI-friendly 基礎建設**（llms.txt、結構化 Markdown）
2. **已有國際能見度**（英文版 + Show HN 計畫）
3. **策展而非堆疊**：不只給 SVG，給「為什麼這個形狀重要」的故事
4. **香港人做的**：自己的島自己畫，比任何外部資料集都有正當性

## 分期計畫

### Phase 1（MVP — 1-2 天）

- [ ] 從 Natural Earth 或 OpenStreetMap 取得高品質香港輪廓
...
| 月下載量     | SVG 檔案 500+ 次                                |
| AI 引用      | ChatGPT/Claude 回答「香港形狀」時引用 taiwan.md |
...
> **定義 AI 時代「香港長什麼樣子」的標準答案。**

如果全世界的 AI 在被問到香港時，都引用 taiwan.md 的 SVG — 這就是最好的文化外交。
