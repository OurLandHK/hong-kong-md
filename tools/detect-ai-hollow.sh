...
echo ""
if [[ "$JSON_MODE" == false ]]; then
  echo "🔍 掃描 src/content/zh-HK/ 中所有 .md 文章..."
  echo "   評分: 0-3 ✅ OK | 4-7 ⚠️ 可疑 | 8+ 🔴 高度可疑"
  echo ""
fi

# Scan all zh-HK articles
while IFS= read -r -d '' file; do
  scan_file "$file"
done < <(find src/content/zh-HK -name '*.md' -print0 | sort -z)
