#!/bin/bash
# HongKong.md Daily Stats & Contributors Update
# Run by cron at midnight daily
set -e
cd "$(dirname "$0")/.."

echo "📊 Updating HongKong.md stats and contributors..."

# 1. Fetch current stats from GitHub API
STARS=$(gh api repos/hongkong-md-org/hongkong-md --jq '.stargazers_count')
FORKS=$(gh api repos/hongkong-md-org/hongkong-md --jq '.forks_count')
CONTRIBUTORS=$(gh api repos/hongkong-md-org/hongkong-md/contributors --jq 'length')
...
CONTRIBUTORS_JSON=$(gh api repos/hongkong-md-org/hongkong-md/contributors --jq '[.[] | {login, avatar: .avatar_url, url: .html_url, contributions: .contributions}]')
