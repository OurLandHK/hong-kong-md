...
console.log('🗺️ HongKong.md Map Marker Generation Script');

// 讀取 geocode 對照表
const geocodeData = JSON.parse(fs.readFileSync(path.join(__dirname, '../src/data/hongkong-geocode.json'), 'utf8'));
const { districts: cities, landmarks } = geocodeData;
...
function generateSlug(category, title) {
  ...
  if (title.includes('茶') && category === 'food') return '/food/香港茶文化';
...
function matchLocations(title, content) {
  ...
  for (const [cityName, cityData] of Object.entries(cities)) {
    ...
    // 例如「台北」要匹配「台北市」「台北」但不能匹配「香港」中的「台」
...
function generateMarkers() {
  ...
  for (const filePath of markdownFiles) {
    try {
      ...
      const description = frontmatter.description || frontmatter.desc || '探索香港的文化與故事';
