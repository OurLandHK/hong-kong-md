...
# 建立目錄結構
echo "📁 建立目錄結構..."
mkdir -p src/content/zh-HK/{about,art,culture,economy,food,geography,history,lifestyle,music,nature,people,society,technology,resources}
...
if [ -f "knowledge/_Home.md" ]; then
    cp "knowledge/_Home.md" "src/content/zh-HK/_Home.md"
    echo "  ✅ _Home.md"
fi
  ...
  if [ -d "knowledge/$category" ]; then
    ...
    for file in knowledge/$category/*.md; do
      if [ -f "$file" ]; then
        ...
        target_file="src/content/zh-HK/$lowercase_category/$filename"
...
for resource_dir in "knowledge/resources" "knowledge/zh-HK/resources"; do
  if [ -d "$resource_dir" ]; then
    for file in $resource_dir/*.md; do
      if [ -f "$file" ]; then
        filename=$(basename "$file")
        target_file="src/content/zh-HK/resources/$filename"
        cp "$file" "$target_file"
