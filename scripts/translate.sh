...
# HongKong.md Translation Contribution Script
# 一行翻譯貢獻自動化系統
# Usage: bash scripts/translate.sh OR bash <(curl -s https://raw.githubusercontent.com/hongkong-md-org/hongkong-md/main/scripts/translate.sh)
...
# Function to check if we're in a HongKong.md repository
check_repo() {
...
setup_repo() {
    print_info "檢查 HongKong.md 環境..."
    
    if check_repo; then
        print_success "已在 HongKong.md 目錄內"
        return 0
    fi
    
    # Not in the right directory, try to clone
    print_warning "不在 HongKong.md 目錄內，準備 clone repository..."
...
get_translation_prompt() {
    ...
    if [[ -f "TRANSLATE_PROMPT.md" ]]; then
        ...
        cat << EOF
# HongKong.md Translation Prompt

> 把這整段貼到 ChatGPT / Claude / Gemini，它會幫你翻譯一篇 HongKong.md 文章。

## 你的角色
你是 HongKong.md 的翻譯志工。HongKong.md 是一個用 Markdown 策展香港知識的開源專案。
...
- **香港專有名詞**：保留中文 + 英文解釋（例：夜市 (night market)）
- **文化脈絡**：不熟悉的概念加簡短解釋
- **策展人聲音**：保持有觀點、有溫度的語氣
...
Created by HongKong.md Translation Automation System."
...
main() {
    ...
    echo -e "${PURPLE}${MAGIC}     🇹🇼 HongKong.md 翻譯貢獻自動化系統 🌐                   ${MAGIC}${NC}"
    echo -e "${PURPLE}${MAGIC}                                                           ${MAGIC}${NC}"
    echo -e "${PURPLE}${MAGIC}     讓你的 AI Token 幫香港被世界看見！                     ${MAGIC}${NC}"
    echo -e "${PURPLE}${MAGIC}                                                           ${MAGIC}${NC}"
    echo -e "${PURPLE}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${MAGIC}${NC}"
    echo ""
    ...
    if [[ $processed_count -gt 0 ]]; then
        ...
        echo ""
        print_magic "感謝你為 HongKong.md 貢獻翻譯！🙏"
