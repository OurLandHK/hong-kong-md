# 👀 Reviewers Guide — HongKong.md 協作者指南

## 角色與權限

HongKong.md 的社群協作分為三個角色：

| 角色 | 權限 | 如何取得 |
|------|------|----------|
| **Contributor** | 提交 PR、回報 Issue | 任何人（Fork → PR） |
| **Reviewer** | Approve PR、管理 Issue labels、建立 branch | 受邀（見下方標準） |
| **Maintainer** | Merge PR、管理 Settings | 僅 @hongkong-md-org |
...
只有 Maintainer（@hongkong-md-org）會執行 merge。Reviewer approve 後，Maintainer 會儘快處理。
...
**Q: Reviewer 有薪水嗎？**
A: 沒有。HongKong.md 是純志工開源專案。我們用 [All Contributors](https://allcontributors.org/) 記錄每位貢獻者。
...
**Q: 如何退出 Reviewer？**
A: 隨時跟 @hongkong-md-org 說一聲，或直接在 GitHub 移除自己的 collaborator 權限。
